...
## 3. [대표 이미지 & 대표이미지 기반 작품묘사] <a name='3'></a>
...
1. 시작화면, 게임화면, 설정화면으로 총 3개의 화면이 있다.  
2. 시작화면에는 시작하기, 설정 2개의 버튼이 있다.  
3. ESC 키를 누르면 설정 화면이 나온다.  
4. ESC 키를 한번 더 누르면 설정 화면이 닫힌다.  
5. 설정화면에서는 사운드 바와 조작법 설명이 있다.  
6. 사운드 바는 마우스로 조절이 가능하다.  
7. 조작법 설명에는 Z, X, R키를 누르면 점프, 공격, 다시하기가 가능하다고 써있다.  
8. 시작하기 키(space bar)를 입력하면 시작화면에서 인게임 화면으로 이동한다.  
9. 게임화면에서는 플레이어 캐릭터가 있으며 ←, →를 통해 좌, 우측으로 이동한다.  
10. 플레이어는 트랩에 닿을 시 바로 사망한다.  
11. 난이도 상승에 따라 몬스터(2스테이지 부터)와 보스(1스테이지 후반부터)가 등장한다.  
12. 몬스터는 이동하고 공격하지만 캐릭터가 몬스터를 처치하는건 불가능하며 회피해야 한다.  
13. 보스는 매 스테이지 후반부 보스룸으로 입장하는 포탈에 플레이어가 닿을 시 보스룸에서 등장한다.  
14. 보스의 체력은 중앙 상단에 체력바(현재 체력은 빨강색, 잃은 체력은 회색)로 확인 가능하다.  
15. 각 스테이지 보스별 공격 패턴은 사망 후 재도전해도 일치하다.  
16. 보스는 총알로 보스를 공격해 보스의 체력이 0이되면 처치가 완료된다.  
17. 조작은 키보드 Z키를 누르면 캐릭터가 위로 1값만큼 점프한다.  
18. 키보드 Z키를 점프상태에서 한 번 더 누르면 캐릭터가 한번 더 점프한다.  
19. 키보드 X키를 누르면 플레이어가 바라보는 위치로 총알을 발사한다.  
20. 키보드 R키를 누르면 사망상태에서 부활하여 게임이 다시시작된다.  
21. 게임이 다시시작 될 때, savepoint 지점에서 부활하게된다.  
22. 첫 시작으로 savepoint가 없을 시 맨 처음부터 시작한다.  
23. 게임화면에서 사망 시 R키를 누르면 재시작이 가능하다는 텍스트가 출력된다.  
24. 게임화면 우측 상단에 현재 스테이지의 정보 텍스트가 출력된다.  
25. 스테이지 정보 텍스트 우측에 현재까지 사망 횟수를 알리는 텍스트가 출력된다.  
26. 발사되는 총알은 0.5초에 한 발씩 충전되며 최대 5발까지 충전된다.
27. 1스테이지 맵의 배경은 우주선 내부 배경이다.
28. 1스테이지 맵에서 사용되는 발판은 우주선 내부 흰색 철제 이미지이다.
29. 인게임에서 사용되는 트랩은 뾰족한 가시 발판이다.
30. 플레이어의 점프는 2회가 최대이며 2단점프 이후 바닥에 닿기 전까지 더 이상 점프가 불가능하다.
31. 보스는 기본공격, 패턴공격1, 패턴공격2, 랜덤공격 총 4종류의 공격이 있다.
32. 보스의 기본공격은 보스가 플레이어 방향으로 근접 혹은 원거리로 공격한다.
33. 보스의 패턴공격1과 2는 정해진 패턴에 따라 공격을 반복한다.
34. 보스의 랜덤공격은 일정한 패턴 없이 무작위로 공격한다.
35. 플레이어는 보스와 충돌 시 사망한다.
36. savepoint는 총알로 피격시 현재 플레이어의 위치값을 저장한다.
37. savepoint에 저장 시, savebox위에 저장되었다는 text를 1초간 출력한다.
38. 1스테이지 보스 맵의 배경은 1스테이지 기본 맵 배경보다 조금 더 어두운 색으로 한다.
39. 캐릭터의 이미지는 우주비행사가 총을 들고있는 이미지이다.
40. 캐릭터의 애니메이션은 회전, 점프, 공격, 사망이 있다.
...
## 12. [프로토타입 개발 요구사항 (6주개발)]<a name='12'></a>  

~~1-1) 게임화면에서는 플레이어 캐릭터가 있으며 →, ←를 통해 좌, 우측으로 이동한다.~~ (9) - 박기현  

~~1-2) 조작은 키보드 Z키를 누르면 캐릭터가 위로 1값만큼 점프한다.~~ (17) - 박기현  

~~1-2) 키보드 Z키를 점프상태에서 한 번 더 누르면 캐릭터가 한번 더 점프한다.~~ (18) - 박기현  

~~1-2) 플레이어의 점프는 2회가 최대이며 2단점프 이후 바닥에 닿기 전까지 더 이상 점프가 불가능하다.~~  (30) - 박기현  

~~1-3) 키보드 X키를 누르면 플레이어가 바라보는 위치로 총알을 발사한다.~~ (19) - 서윤성  

~~1-3) 발사되는 총알은 0.5초에 한 발씩 충전되며 최대 5발까지 충전된다.~~ (26) - 서윤성  
 
 <br> 

~~2-1) 설정화면에서는 사운드 바와 조작법 설명이 있다.~~ (5) - 서윤성  

~~2-1) 조작법 설명에는 Z, X, R키를 누르면 점프, 공격, 다시하기가 가능하다고 써있다.~~ (7) - 서윤성  

~~2-2) ESC 키를 누르면 설정 화면이 나온다.~~ (3) - 박기현  

~~2-2) ESC 키를 한번 더 누르면 설정 화면이 닫힌다.~~ (4) - 박기현  

~~2-3) 게임화면에서 사망 시 R키를 누르면 재시작이 가능하다는 텍스트가 출력된다.~~ (23) - 서윤성  

~~2-4) 스테이지 정보 텍스트 우측에 현재까지 사망 횟수를 알리는 텍스트가 출력된다.~~ (25) - 박기현  

 <br> 

~~3-1) 1스테이지 맵의 배경은 우주선 내부 배경이다.~~ (27) - 박기현, 서윤성

~~3-1) 1스테이지 맵에서 사용되는 발판은 우주선 내부 흰색 철제 이미지이다.~~ (28) - 박기현, 서윤성

~~3-2) 플레이어는 트랩에 닿을 시 사망한다.~~ (10) - 박기현

~~3-3) 게임화면 우측 상단에 현재 스테이지의 정보 텍스트가 출력된다.~~ (24) - 박기현

~~3-4) 캐릭터 이미지와 회전을 구현한다.~~ (39, 40) - 서윤성

 <br> 

~~4-1) savepoint는 총알로 피격시 현재 피격한 플레이어의 위치값을 저장한다.~~ (36) - 서윤성

4-1) savepoint에 저장 시, savebox위에 저장되었다는 text를 1초간 출력한다. (37) - 서윤성

~~4-2) 키보드 R키를 누르면 사망상태에서 부활하여 게임이 다시시작된다.~~ (20) - 박기현

~~4-3) 게임이 다시시작 될 때, savepoint 지점에서 부활하게된다.~~ (21) - 박기현

~~4-3) 첫 시작으로 savepoint가 없을 시 맨 처음부터 시작한다.~~ (22) - 박기현

4-4) 캐릭터의 점프 애니메이션을 구현한다. (40) - 서윤성 (92%)

 <br> 

5-1) 1스테이지 보스 맵의 배경은 1스테이지 기본 맵 배경보다 조금 더 어두운 색으로 한다. (38) - 서윤성 (80%)

~~5-2) 플레이어는 보스와 충돌 시 사망한다.~~ (35) - 박기현

~~5-4) 보스는 매 스테이지 후반부 보스룸으로 입장하는 포탈에 플레이어가 닿을 시 보스룸에서 등장한다.~~ (13) - 박기현

5-5) 캐릭터의 공격 애니메이션을 구현한다. (40) - 서윤성 (55%)

 <br> 

6-1) 보스의 체력은 중앙 상단에 체력바(현재 체력은 빨강색, 잃은 체력은 회색)로 확인 가능하다. (14) - 서윤성

6-2) 보스의 패턴공격 1을 구현한다. (33) - 박기현

6-3) 보스의 패턴공격 2를 구현한다. (33) - 서윤성

6-4) 캐릭터의 사망 애니메이션을 구현한다. (40) - 서윤성



## 13. [작업 결과]<a name='13'></a>
